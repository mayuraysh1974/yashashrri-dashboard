...
const TestScheduler = () => {
  ...
  const addToCalendar = (test) => {
    ...
    const subjects = Array.isArray(test.subjects) ? test.subjects.join(', ') : test.subject;
    const std = test.standard || (Array.isArray(test.standards) ? test.standards[0] : 'N/A');
    
    // Create ICS content with 15-minute reminder (VALARM)
    const icsContent = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Yashashrri Dashboard//EN',
      'BEGIN:VEVENT',
      `UID:${test.id}@yashashrri.com`,
      `DTSTAMP:${new Date().toISOString().replace(/[-:]/g, '').split('.')[0]}Z`,
      `DTSTART;VALUE=DATE:${dateStr}`,
      `DTEND;VALUE=DATE:${dateStr}`,
      `SUMMARY:Test: ${test.name} (${std})`,
      `DESCRIPTION:Subjects: ${subjects}\\nTotal Marks: ${test.total_marks || test.totalMarks}\\n\\nGenerated by Yashashrri Dashboard`,
      'LOCATION:Yashashrri Classes',
      'BEGIN:VALARM',
      'TRIGGER:-PT30M',
      'ACTION:DISPLAY',
      'DESCRIPTION:Upcoming Test Reminder',
      'END:VALARM',
      'END:VEVENT',
      'END:VCALENDAR'
    ].join('\r\n');
    ...
    link.href = url;
    link.setAttribute('download', `Test_${test.name.replace(/\s+/g, '_')}.ics`);
  ...
  const requestNotifications = async () => {
    ...
    if (permission === 'granted') {
        alert('Browser notifications enabled! You will now receive alerts for reminders.');
    }
  };
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button className="btn-secondary" onClick={requestNotifications} style={{ border: '1px solid #E2E8F0' }}>
          ...
          return (
            ...
            >
              <div style={{ position: 'absolute', top: '1.25rem', right: '1.25rem', display: 'flex', gap: '0.5rem' }}>
                 <button className="btn-secondary" style={{ padding: '0.45rem', borderRadius: '8px', border: '1px solid #E2E8F0', background: 'white' }} onClick={() => addToCalendar(test)} title="Add to My Calendar / Set Alarm">
                   <FiCalendar size={14} color={classColors.primary} />
                 </button>
                 <button className="btn-secondary" style={{ padding: '0.45rem', borderRadius: '8px', border: '1px solid #E2E8F0', background: 'white' }} onClick={() => sendTestAlert(test)} title="Send Student SMS Alert">
